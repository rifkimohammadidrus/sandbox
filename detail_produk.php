...
// diskon persentase cekout Langsung
  ...
  ?>
	<?php 
		...
		// echo $qharga."<br><br>";
		list($harga1)=mysqli_fetch_array($query_harga);            
				
		$q_disc="SELECT tdd.persen_diskon,tdd.potongan_harga,td.start,td.end  FROM tbl_disc AS td 
						LEFT JOIN tbl_disc_detail AS tdd ON (tdd.id_diskon = td.id)
						WHERE  tdd.barcode='$idb'  and td.outlet like '%$id_outlet%' AND td.status=1";
		$query_disc=mysqli_query($connect,$q_disc); 
		list($persen_disc,$potongan_harga,$tglawal,$tglakhir)=mysqli_fetch_array($query_disc);
		
		// cek apakah tanggal skg masih dalam periode diskon
		$now=date('Y-m-d');	
		if (($now>=$tglawal) and ($now<=$tglakhir) and $persen_disc!=0){
			$nilai_diskon=($persen_disc/100)*$harga1;
			$harga_diskon=$harga1-$nilai_diskon;
		} elseif(($now>=$tglawal) and ($now<=$tglakhir) and $potongan_harga!=0) {
			$harga_diskon=$harga1-$potongan_harga;
		}else{
			$persen_disc=0;	
			$potongan_harga=0;
			$harga_diskon=$harga1;
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form method="post" id="formPesanLangsung">
						...
						<input type="hidden" name="disc" id="disc" value="<?php echo $persen_disc ;?>">
						<input type="hidden" name="potongan_harga" id="potongan_harga" value="<?php echo $potongan_harga ;?>">
