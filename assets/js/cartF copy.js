...
function hitungTotal(){
	...
	try{
		$('.subtotal').each(function(index, element) {
			total+=Number(removeDots($('#'+this.id).text()));
		});
		// alert(total);
		console.log(total);
		$('#total').text(format('#,##0.##',total));
			...
			success: function(data) {	
			   ...
			   sisa++;	
				try{				
					...
					$('#am_'+sku+'_'+store).text('Rp. '+ format('#,##0.#',data.amount));
					$('#cek_'+sku+'_'+store).attr('data-id',data.amount);
