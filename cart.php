...
<?php //include("template/kode_beli.php"); ?>
<?php 


// $session_kode_beli= session_id();
$session_kode_beli= $_COOKIE['kode_beli'];

// echo $session_kode_beli;
// die;
...
<style>
	...
	.total_cart{
		float:left; margin-top:-10px; margin-left:8px; font-family: 'Roboto', sans-serif; color: #b75ae2;
	}
	.span_rp{
		float:left;  margin-left:9px; margin-top:-8px
	}
	@media (max-width: 767px){
		...
		.total_cart{
			float:left; margin-top:-10px; margin-left:14px; font-family: 'Roboto', sans-serif; color: #b75ae2;
		}
		.span_rp{
			float:left;  margin-left:16px; margin-top:-8px
		}
	}
		...
		<div id="mobile" class="container clearfix">
			...
			<?php if ($rowcount !=0) {
				...
				while(list($id,$nama,$ukuran,$warna,$gambar,$harga,$qty,$amount,$berat,$kodejenis,$kodeukuran, $kodewarna,$diskon,$kode_diskon,$id_outlet,$sisa_stok)=mysqli_fetch_array($query)){
					...
					$nilai_diskon=($persentase_diskon/100)*$harga;
					$harga_diskon=$harga-$nilai_diskon; 
					
					$en_outlet=encrypt($id_outlet);
					?>
					<div class="row" style="border-top:2px solid #d9d9d9;">
						<div id="keranjang"class="col-auto mr-auto mt-2 mb-2" >
							<input class="checkItem check" type="checkbox" id="am_<?php echo $id.'_'.$id_outlet;?>" name="check" value="<?php echo $subtotal;?>" data-id="<?php echo $id ?>">

							<!-- <img class="img-keranjang" src="assets/foto_produk/thumbnail/<?= $gambar?>" alt="<?= $nama;?>">	 -->
							<img class="img-keranjang" src="assets/foto_produk/<?= $gambar?>" alt="<?= $nama;?>">	
						</div>
	
						<div id="mob-produk-keranjang" class="col mt-1 mb-1">
							...
							<p class="harga-promo-keranjang" >
								<?php if ($persentase_diskon !=0) {
									...
									?>
								<span class="disc"id="dc_<?= $id.'_'.$id_outlet;?>"><?= round($persentase_diskon,2)." %"; ?></span> 
								<!-- <span class="disc"id="dc_<?= $id.'_'.$id_outlet;?>"><?= round($persentase_diskon,2)." %"; ?></span>  -->
							
								<span class="harga-awal">Rp. <?= number_format($harga,"0",".",",");?></span> &nbsp;
								<?php } ?>
								<span class="harga-disc"id="pr_<?= $id.'_'.$id_outlet;?>">Rp. <?= number_format($harga_diskon,"0",".",",");?></span> 
					...
					<input type="hidden" name="outlet" id="outlet" value="<?php echo $id_outlet ?>">
					<?php 
				...
				<div class="row" style="border-top:2px solid #d9d9d9;">
					<div id="mob-produk-keranjang " class="col mt-3 center"> 
						...
						<br><span class="span_rp"><h4 class="rp" id="total"> Rp. 0 </h4></span>
						
						<button type="button" class="btn btn-outline-secondary btn-cekout" id="d1" >Checkout</button>
					</div>
				</div>
				<br>
				<!-- .postcontent end -->
					...
					<div id="mobile" class="container">
						...
						<form id="formAdd" action="" method="post">
							...
							<div class=" mt-4">
								...
								<input type="submit" id="lanjut_pembelian" class="btn btn-outline-secondary btn-custom" style="float:right" value="Lanjut Pembelian">
							</div>
						</form>
					</div>
				</div>
					...
					<div id="mobile" class="container">
						<h4 class="cekout-proses">Pesanan Diproses</h4>
							<div id="mob-produk-cekout" class="col-auto ml-auto mt-3">
								...
								<input type="hidden" id="outlet_id" name="outlet_id" value="<?php echo $en_outlet ?>">
								<input type="hidden" id="outlet_name" name="outlet_name" value="<?php echo $nama_outlet ?>">
							</div>
